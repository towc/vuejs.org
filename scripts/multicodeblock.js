// taken from https://github.com/towc/hexo-multiple-codeblock
// but put here so that it's easier to maintain for later
...
const parse = ([defaultLanguage], content) => {
  ...
  const htmlParts = objectParts.map((objectPart) => {
    ...
    return `${codeblockTagName} slot="${objectPart.config.slot}">${codeblockTagParts.join('>')}\n${content.join('\n')}`
  });
